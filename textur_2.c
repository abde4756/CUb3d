...
/* ========== FONCTION 1: Convertir RGB en couleur int ========== */
...
static void	init_render_data(t_map *map, t_dda *dda, t_textr *tex, t_render *r)
{
	...
	r->ceiling_color = rgb_to_int(map->file->color.ceiling);
	r->floor_color = rgb_to_int(map->file->color.floor);
	r->tex_x = calcul_position_texture(dda, map, tex);
	r->step = (double)tex->height_txtr / (double)r->line_height;
...
static void	draw_wall_texture(int x, t_map *map, t_textr *tex, t_render *r)
{
	int		y;
	int		tex_y;
	int		color;
	...
	while (y <= r->draw_end)
	{
		...
		if (tex_y < 0)
			tex_y = 0;
		if (tex_y >= tex->height_txtr)
			tex_y = tex->height_txtr - 1;
		r->tex_pos += r->step;
		pixel = tex->adrs + tex_y * tex->octe_ligne + r->tex_x * (tex->bit_pxl
				/ 8);
		color = *(int *)pixel;
